 ...
 * @return string
 */
function blogtwist_hybrid_media_grabber( $args = array() ) {

	$media = new Blogtwist_Hybrid_Media_Grabber( $args );
 ...
 * @return void
 */
class Blogtwist_Hybrid_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'blogtwist_hybrid_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'blogtwist_hybrid_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed' ) );
		...
		$this->args    = apply_filters( 'blogtwist_hybrid_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'blogtwist_hybrid_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'blogtwist_hybrid_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'blogtwist_hybrid_media_grabber_autoembed_media', $value[0] ) );
	...
	public function filter_dimensions( $html ) {
		...
		if (isset($media_atts['width']) && isset($media_atts['height']) && is_numeric($media_atts['width']) && is_numeric($media_atts['height'])) {
		...
		list( $width, $height ) = apply_filters(
			'blogtwist_hybrid_media_grabber_dimensions',
